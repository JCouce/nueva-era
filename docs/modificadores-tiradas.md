...
son diez minutos y evita reinventar uno de los tres mecanismos que ya
existen.
...
## 2. Los tres mecanismos que SÍ dependen de la tirada en concreto
...
elige algo antes de tirar, o de qué arma en concreto la generó. Para eso hay
tres mecanismos, todos en `src/lib/rules/condiciones.ts` y usados desde
`combate.ts`:
...
Los tres se pintan en el mismo sitio: el bloque **"// Desglose"** de
...
**Por qué tres y no uno.** Cada uno resuelve una pregunta distinta:
"¿decide el jugador?" (condición), "¿depende de otra elección ya hecha?"
(bonoTramo), "¿es siempre igual mientras esté puesto?" (ajusteFijo). Forzar
los tres a converger en un único mecanismo genérico sería más "elegante" en
abstracto, pero cada uno es hoy una función de 5 líneas, fácil de leer y de
testear — no compensa la ceremonia de unificarlos.

## 3. El árbol de decisión: qué mecanismo usar

Cuando añadas un modificador nuevo a una mejora de equipo, hazte estas
preguntas en orden:
...
1. **¿El jugador tiene que decidir algo en el momento de tirar** (sí/no, una
   ...
   **No hace falta tocar nada más.** El modal ya sabe dibujar los tres tipos
   ...
   tipos existan: cerrar la lista a tres formas conocidas es lo que permite
   que una mejora nueva sea solo datos, nunca código nuevo.

2. **¿Es automático, no depende de nada, y es siempre el mismo número
   mientras la pieza está puesta?** → **`ajustesFijos`** (campo
   `ajusteAtaque` del nivel). Aparece en el desglose con la etiqueta de la
   pieza que lo trae.

3. **¿Es automático, pero cambia según el tramo de distancia que el jugador
   ya eligió?** → **`bonosTramo`** (campo `ajusteTramo` del nivel).
...
4. **¿Es un bono de personaje entero, no ligado a un arma o pieza
   ...
   frío...)? → Ver la sección 5. **Hoy este caso no funciona.**
...
## 5. El grifo que NO funciona todavía (y por qué está documentado, no arreglado)

`Modificador` tiene un cuarto tipo, `"tirada"`, con un `contexto: string`
libre — pensado para bonos de personaje entero: las salvaciones de
congelación/llamarada del traje, el "+2 ocultar arma" de la Mosquito, las
resistencias psiónicas de Arkorü. Hay **31 sitios en el catálogo** que lo
declaran.

**Ahora mismo no llega a ninguna tirada.** Existe una función,
`bonoTirada()` en `modificadores.ts`, que sabría sumarlos, pero nada la
llama desde `tiradas.ts` ni desde el modal. No es un bug de esta sesión —
llevaba así desde antes; solo se ha hecho visible ahora que el desglose
muestra de verdad todo lo que falta.

**Por qué no se ha enganchado sin más:** el `contexto` es texto libre con
comparación aproximada (`contexto.includes()` en los dos sentidos). Engan­
charlo tal cual metería, por ejemplo, el bono del Sistema de Retroceso en
**todos** los modos de disparo del arma, no solo en el automático — el mismo
tipo de bug que ya se corrigió dos veces con el lanzagranadas y la mira,
disfrazado de otra forma.

**El arreglo diseñado** (pendiente de implementar): sustituir el `contexto`
libre por un alcance cerrado y tipado —

```ts
type AlcanceModificador =
  | { tipo: "grupo"; grupo: Tirada["grupo"] }        // todas las de un grupo (Salvaciones, Acciones...)
...
— y una función de resolución (`bonoAlcance`, al lado de
`desgloseCondiciones`) que el modal llame igual que llama a las otras tres.
Migrar los 31 sitios es mecánico una vez decidido el alcance de cada uno.

**Si necesitas uno de estos 31 modificadores HOY**, no asumas que funciona:
compruébalo abriendo el modal y mirando el desglose. Si no aparece, es este
grifo.

## 6. Ejemplo trazado de punta a punta: el Bípode

Para ver los tres mecanismos con código real, sigue el Bípode:
...
3. ¿Es un `Modificador` de tipo `"tirada"`? **No funciona todavía** — ver
   sección 5. No es un bug, es el grifo pendiente.
