...
3. **`docs/tareas.md`, entero** — qué está hecho, qué falta y qué lo bloquea. Es la
   única fuente de estado del proyecto: si algo de lo que sigue en este encargo
   contradice a `docs/tareas.md`, gana `docs/tareas.md` — este documento no se
   actualiza tan seguido.
4. **`docs/traspaso.md`, entero** — cómo se trabaja aquí, trampas que ya han mordido a
   alguien, mapa de archivos. Su §9 tiene el razonamiento de diseño de la fase de
   máster y creación por prioridad, que es lo más grande construido hasta ahora.
5. **`docs/sistema.md`** — la única fuente de verdad de las reglas del juego. Si una
...
| `src/lib/rules/aprobacion.ts` | El guardarraíl de aprobación (snapshot, suelo). Ojo: sigue viva pero `characters/[id]/actions.ts` ya no la usa para atributos/habilidades — ver por qué en `docs/traspaso.md` §9. |
...
| `prisma/schema.prisma` | `Character.stats` (Json, la ficha entera) + `status`/`approvedAt`/`approvedSnapshot`/`xp`/`creditos` como columnas propias — deliberado, ver por qué en `docs/traspaso.md` §9. |

## Estado exacto ahora mismo

**No lo repito aquí — está en `docs/tareas.md`, y solo ahí, precisamente para que esta
sección no se quede desfasada como le pasó antes** (llegó a decir que la Tienda no
cobraba créditos reales cuando ya llevaba un tiempo cobrándolos). Léelo entero: tiene
hecho/pendiente por fase, lo que quedó fuera a propósito y las preguntas al diseñador
por impacto.
...
La lista priorizada vive en la sección "Pendiente" de `docs/tareas.md` — no la repito
aquí para no volver a desincronizarla. No empieces ninguna por tu cuenta: pregunta
prioridad primero, esa lista es un inventario, no una cola de trabajo.

## Cómo quiero que empieces

1. Lee los cinco documentos de arriba, en ese orden.
